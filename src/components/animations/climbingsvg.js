...
const ClimbingSvg = React.forwardRef((props, ref) => (
  ...
  >
    ...
    <path
      ...
      ref={ref.rope_ref}
    />
    <circle cx={213} cy={270} r={7.537} css="fill:#d8d8d8" ref={ref.head_ref} />
    <ellipse
      cx={213.912}
      cy={295.402}
      rx={10.769}
...
const top_blue_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 70, y: 50 }, 0.5, 0.9)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const top_orange_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 100, y: 0 }, 0.1, 0.6)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const top_pink_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 30, y: -50 }, 0, 0.8)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const lower_orange_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 70, y: -50 }, 0.36, 0.9)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const upper_red_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 34, y: -10 }, 0.5, 0.9)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const lower_red_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 60, y: 50 }, 0.1, 0.9)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const bottom_blue_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 70, y: 35 }, 0.5, 0.9)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const bottom_green_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 40, y: -40 }, 0.5, 0.9)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const wall_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: -100, y: -40 }, 0, 0.9)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
const person_animation = (ref, ratio) => {
  let curr = linearAnimation(ratio, { x: 10, y: -120 }, 0.1, 0.9)
  ref.current.style.transform = `translate(${curr.x}px, ${curr.y}px)`
}
